...
resource "aws_route_table_association" "spoke_4_wan1_RT_association" {
  provider       = aws.spoke_4_vpc
  subnet_id      = aws_subnet.spoke_4_wan1_subnet.id
...
resource "aws_route_table_association" "spoke_4_wan2_RT_association" {
  provider       = aws.spoke_4_vpc
  subnet_id      = aws_subnet.spoke_4_wan2_subnet.id
  route_table_id = aws_default_route_table.public_subnets_RT.id
}
...
resource "aws_route_table_association" "spoke_4_private_RT_association" {
  provider       = aws.spoke_4_vpc
  subnet_id      = aws_subnet.spoke_4_private_subnet.id
  route_table_id = aws_route_table.spoke_4_pvt_rt.id
}