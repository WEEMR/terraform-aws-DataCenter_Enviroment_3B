...
resource "aws_network_interface" "spoke1_Ubuntu_WebServer_eth0" {
  ...
  description = "${var.username}_Terraform_spoke_2_Apache_eth0"
  subnet_id   = var.spoke_2_private_subnet_id
  private_ips = var.spoke_2_Ubuntu_LAN_IP
  security_groups = [var.Private_SG]

  tags = {
    Name = "${var.username}_TF_spoke_2_Ubuntu_WebServer_Eth0"
  }
}
...
resource "aws_instance" "spoke_2_Apache" {
  provider          = aws.spoke_2_vpc
  ...
  #
  echo "<html><style>body { font-size: 15px;}</style><body><h1>Hello, Everyone &#128075</h1><h2>This is our Spoke 2 Apache Server created via Terraform &#128079 &#128170; </h2></body></html>" > /var/www/html/index.html
  #
  #  Install Net Tools
  #
  ...
  tags = {
    Name = "${var.username}_TF_spoke_2_Ubuntu_Apache_Server"
  }
}
