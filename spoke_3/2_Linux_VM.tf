...
resource "aws_network_interface" "spoke1_Ubuntu_WebServer_eth0" {
  ...
  description = "${var.username}_Terraform_spoke_3_Apache_eth0"
  subnet_id   = var.spoke_3_private_subnet_id
  private_ips = var.spoke_3_Ubuntu_LAN_IP
  security_groups = [var.Private_SG]

  tags = {
    Name = "${var.username}_TF_spoke_3_Ubuntu_WebServer_Eth0"
  }
}
...
resource "aws_instance" "spoke_3_Apache" {
  provider          = aws.spoke_3_vpc
  ...
  #
  echo "<html><style>body { font-size: 15px;}</style><body><h1>Hello, Everyone &#128075</h1><h2>This is our Spoke 3 Apache Server created via Terraform &#128079 &#128170; </h2></body></html>" > /var/www/html/index.html
  #
  #  Install Net Tools
  #
  ...
  tags = {
    Name = "${var.username}_TF_spoke_3_Ubuntu_Apache_Server"
  }
}
