...
# Activate VPC Flow Logs on Spoke 2

resource "aws_flow_log" "Spoke2_vpc" {
  provider        = aws.virginia
  iam_role_arn    = aws_iam_role.SDWAN_Lab_IAM_Role.arn
  log_destination = aws_cloudwatch_log_group.SDWAN_Lab_VPCs_Logs.arn
  traffic_type    = "ALL"
  vpc_id          = module.Spoke_2_Prod_VPC.SDWAN_VPC
}

# Activate VPC Flow Logs on Spoke 3

resource "aws_flow_log" "Spoke3_vpc" {
  provider        = aws.virginia
  iam_role_arn    = aws_iam_role.SDWAN_Lab_IAM_Role.arn
  log_destination = aws_cloudwatch_log_group.SDWAN_Lab_VPCs_Logs.arn
  traffic_type    = "ALL"
  vpc_id          = module.spoke_3_Prod_VPC.SDWAN_VPC
}

# Activate VPC Flow Logs on Spoke 4

resource "aws_flow_log" "Spoke4_vpc" {
  provider        = aws.virginia
  iam_role_arn    = aws_iam_role.SDWAN_Lab_IAM_Role.arn
  log_destination = aws_cloudwatch_log_group.SDWAN_Lab_VPCs_Logs.arn
  traffic_type    = "ALL"
  vpc_id          = module.spoke_4_Prod_VPC.SDWAN_VPC
}